...
def make_taylor(ref, models, labels, ax=None):
    ...
    td, _, _, _ = taylor_diagram(
        np.std(ref, ddof=1), fig=ax.figure, rect=111, label="REF"
    )
...
def plot_change_map(lat, lon, y0, y1_obs, y1_pred, species_id, extent=EUROPE):
    proj = ccrs.PlateCarree()
    fig, axs = plt.subplots(1, 4, figsize=(18, 4), subplot_kw={"projection": proj})
    titles = [f"Year-0 obs", "Year-1 obs", "Year-1 pred", "Pred - obs (yr-1)"]
    mats = [y0, y1_obs, y1_pred, y1_pred - y1_obs]
    for ax, mat, ttl in zip(axs, mats, titles):
        ax.set_extent(extent, crs=proj)
        ax.coastlines("50m")
        mesh = ax.pcolormesh(lon, lat, mat, cmap="viridis", transform=proj)
        ax.set_title(ttl)
...
def plot_hexbin(all_pred, all_obs):
    ...
    hb = ax.hexbin(all_obs, all_pred, gridsize=80, bins="log", mincnt=1, cmap="viridis")
...
def plot_confusion_map(lat, lon, y_true, y_pred, thresh=1):
    ...
    ax.set_extent(EUROPE, crs=proj)
    ax.coastlines("50m")
    mesh = ax.pcolormesh(lon, lat, cats, cmap=cmap, transform=proj, vmin=0, vmax=3)
    cb = fig.colorbar(mesh, ticks=[0.5, 1.5, 2.5, 3.5], shrink=0.7)
    cb.ax.set_yticklabels(labels)
    ax.set_title("Presence/absence confusion")
    plt.show()


def plot_error_violin(errors_per_species):
    fig, ax = plt.subplots(figsize=(10, 4))
    sns.violinplot(data=errors_per_species.T, cut=0, bw=0.2, ax=ax, inner="quartile")
    ax.set_xlabel("Species index")
    ax.set_ylabel("|Pred - Obs|")
    ax.set_title("Error distribution per species")
    plt.show()


def plot_taylor_single(
    obs: np.ndarray, pred: np.ndarray, title: str = "Taylor diagram"
):
    """
