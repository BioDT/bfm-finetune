...
def coarse_grid(data, grid_lat, grid_lon):
    return (
        data.groupby_bins("y", grid_lat, labels=grid_lat[:-1])
        .mean()
        ...
        if hasattr(tas, "rio") and hasattr(pr, "rio"):
            if hasattr(tas.rio, "equals") and callable(tas.rio.equals):
                if not tas.rio.equals(pr):
                    print(
                        f"Warning: Mismatch in geotransform between {tas_path} and {pr_path}"
                    )
            else:
                # Alternative check: compare basic geospatial attributes
                tas_attrs = {
                    k: getattr(tas, k)
                    for k in ["transform", "crs", "res"]
                    if hasattr(tas, k)
                }
                pr_attrs = {
                    k: getattr(pr, k)
                ...
                if tas_attrs != pr_attrs:
                    print(
                        f"Warning: Mismatch in geospatial attributes between {tas_path} and {pr_path}"
                    )
...
def main(cfg: DictConfig):
    ...
    elif hasattr(model.backbone, "use_checkpoint") and hasattr(
        model.backbone.use_checkpoint, "__call__"
    ):
    ...
    lat_bins = np.round(
        np.arange(cfg.grid.lat_start, cfg.grid.lat_end + 1e-6, cfg.grid.resolution), 3
    )
    lon_bins = np.round(
        np.arange(cfg.grid.lon_start, cfg.grid.lon_end + 1e-6, cfg.grid.resolution), 3
    )
    ...
    print(
        f"Processing {len(valid_dates)} valid dates in range {start_year}-{start_month:02d} to {end_year}-{end_month:02d}..."
    )

    for batch_idx, (year, month) in tqdm(
        enumerate(valid_dates), total=len(valid_dates), desc="Processing datasets"
    ):
        batch = next(
            iter(dataloader)
        )  # Just get the first batch since we're loading files manually
        ...
        try:
            ...
            if batch_idx < len(predictions) and predictions[batch_idx]:
                ...
                if "pred" in pred_data:
                    print(
                        f"Keys in predictions[batch_idx][0]['pred']: {list(pred_data['pred'].keys())}"
                    )
            else:
                print(
                    f"Warning: No prediction data available for batch_idx={batch_idx} ({year}-{month:02d})"
                )
            ...
            try:
                ...
                if (
                    pred_data
                    and "pred" in pred_data
                    and "climate_variables" in pred_data["pred"]
                ):
                    ...
                    if (
                        isinstance(decoded_t2m, torch.Tensor)
                        and decoded_t2m.dtype == torch.bfloat16
                    ):
                        decoded_t2m = decoded_t2m.to(torch.float32).cpu().numpy()
                    if (
                        isinstance(decoded_pr, torch.Tensor)
                        and decoded_pr.dtype == torch.bfloat16
                    ):
                    ...
                    decoded_flat = np.stack([decoded_t2m, decoded_pr], axis=-1).reshape(
                        -1, 2
                    )
                else:
    ...
    if not all_latents:
        raise ValueError(
            "No valid data was processed. Check your file paths and date formats."
        )

    print(
        f"Successfully processed {len(all_latents)} dates. Creating output dataset..."
    )
    ...
    try:
        ...
        actual_pixels = (
            ...
            else latent_arr.shape[1] * latent_arr.shape[2]
        )
        ...
        if len(latent_arr.shape) == 3 and latent_arr.shape[1] == expected_pixels:
            ...
            print(
                f"Expected {expected_pixels} pixels for {n_lats}x{n_lons} grid, but got {actual_pixels}. Using flat representation."
            )
    except Exception as e:
    ...
    if use_flat_representation:
        ...
        if len(latent_arr.shape) != 3:
            print(
                f"Warning: Expected 3D array for reshaping to (time, lat, lon, embedding_dim) but got shape {latent_arr.shape}"
            )
            ...
            ds_model = xr.Dataset(
                {
                    "encoder_output": (
                        ["time", "pixel", "embedding_dim"],
                        latent_arr.reshape(
                            latent_arr.shape[0], -1, latent_arr.shape[-1]
                        ),
                    ),
                    "backbone_output": (
                        ["time", "pixel", "embedding_dim"],
            ...
            ds_model = xr.Dataset(
                {
                    "encoder_output": (
                        ["time", "lat", "lon", "embedding_dim"],
                        latent_arr.reshape(
                            latent_arr.shape[0], n_lats, n_lons, latent_arr.shape[2]
                        ),
                    ),
                    "backbone_output": (
                        ["time", "pixel", "embedding_dim"],
